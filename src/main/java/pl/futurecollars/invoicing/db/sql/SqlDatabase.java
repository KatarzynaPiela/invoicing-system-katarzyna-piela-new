...
public class SqlDatabase implements Database {
  ...
  private Integer insertCardAndGetItId(Car car) {
    if (car == null) {
      return null;
    }
  ...
  public Optional<Invoice> update(int id, Invoice updateInvoice) {
    ...
    updateCompany(updateInvoice.getBuyer(), originalInvoice.get().getBuyer());
    updateCompany(updateInvoice.getSeller(), originalInvoice.get().getSeller());
    ...
    addEntriesRelatedToInvoice(id, updateInvoice);
  ...
  private void addEntriesRelatedToInvoice(int invoiceId, Invoice invoice) {
    ...
    invoice.getEntries().forEach(entry -> {
      jdbcTemplate.update(connection -> {
        ...
        ps.setObject(6, insertCardAndGetItId(entry.getExpenseRelatedToCar()));
        return ps;
      }, keyHolder);
